...
#This class will have methods (functionalities like deposit, withdrawal,transfer,check balance, etc)
#create a copy of class in memory and pass it in a category as an argument. Categories are food, clothing, etc
...
def new_budget():
    ...
    budget_title = input("What is the name of this budget? \n")
    try:
        amount = int(input("How much would you like to put in? \n$"))
    except:
        print("\nInvalid input")
        new_budget()
...
def debit():
    ...
    if (pick == 1):
        ...
        print("\nYou have ended the debit transaction ")
        menu()
    else:
        print("\nInvalid option")
        debit()
...
def credit():
    print("**** Deposit money into a budget ****\n")
    print("**** Available Budgets ****")
    for key, value in database.items():
        print(f"- {key}")
    pick = int(input("Press (1) To continue with your deposit transaction\nPress (2) To stop deposit transaction\n"))
    if (pick == 1):
...
def transfer():
    ...
    if from_budget in database:
        ...
        if from_amount < database[from_budget]:
            ...
            print(f"You do not have up to -${from_amount} in {from_budget} budget")
            transfer()
    else:
...
def out():
    try:
        pick = int(input("Are you done planning your budget? \nPress (1) Yes \nPress (2) No\n"))
    except:
        print("Invalid option \n")
        out()

    if (pick == 1):
        print("\nGreat job on planning your finances, good bye.")
        quit()
    elif (pick == 2):
        menu()
    else:
